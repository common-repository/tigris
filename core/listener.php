...
function acs_tfs_replace_entity($content,$tag)
{
...
function acs_tfs_save_listener($options)
{
	$secure = false;
	if (!isset($options['channel'])){
		acs_tfs_respond_sf('false');
	...
	$content=acs_tfs_replace_entity($content,'Tigris__Vacature_omschrijving__c');
	$content=acs_tfs_replace_entity($content,'Tigris__Introductie__c');
	$content=acs_tfs_replace_entity($content,'Tigris__Geboden_wordt__c');
	$content=acs_tfs_replace_entity($content,'Tigris__Gevraagd_wordt__c');
	$content=acs_tfs_replace_entity($content,'Tigris__Bedrijfsomschrijving__c');
	$content=acs_tfs_replace_entity($content,'Tigris__Valuta__c');
		...
		foreach ($data['data'] as $key => $value){
			...
			if ($value['actiontype'] == 'insert') {
				...
				if(isset($value['tigris__owner_name__c']) && !isset( $options['roles_off'] )){
					$post_data['post_author'] = acs_tfs_checked_tigris_author( $value['tigris__owner_name__c'] );
				}
				...
				if(isset($value['tigris__branche__c']) && !isset( $options['category_off'] )){
					$cat_name = acs_tfs_crate_vacancies_category( $value['tigris__branche__c'] );
				...
				if(isset($value['tigris__soort_vacature__c'])){
					$tags[] = __( $value['tigris__soort_vacature__c'] , ACS_TFS_NAME );
				}
				if(isset($value['tigris__interne_vacature__c'])){
					$tags[] = __( 'interne', ACS_TFS_NAME );
				}
				if(isset($value['tigris__plaats__c'])){
					$tags[] = __( $value['tigris__plaats__c'], ACS_TFS_NAME );
				}

				if (!empty($tags)){
					acs_tfs_add_tigristag( $post_id, $tags );
				}
				...
				do_action( 'acs_hook_tigris_insert', $value );
			}
			...
			if ($value['actiontype'] == 'update'){
				...
				elseif(!empty($get_post) && isset($value['tigris__offline__c']) && $value['tigris__offline__c'] == 'true'){
					...
					// update or new Author in Database
					if(isset($value['tigris__owner_name__c']) && !isset( $options['roles_off'] )){
						$post_data_new['post_author'] = acs_tfs_checked_tigris_author( $value['tigris__owner_name__c'] );
					}
					elseif (!isset( $options['roles_off'])) {
					...
					acs_tfs_update_tigristag( $get_post[0]->ID, $tags );
				...
				do_action( 'acs_hook_tigris_update', $value );
			...
			if ($value['actiontype'] == 'delete'){
				...
				do_action( 'acs_hook_tigris_delete', $value);
	...
	// Respond
	if ($retVal) {
		acs_tfs_respond_sf('true');
	}else{
		acs_tfs_respond_sf('false');
	}
}

function acs_tfs_respond_sf($tf) {
if ($tf == 'true')
