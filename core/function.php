...
/**
 * [acs_tfs_init_translation			CORE: Activate translation hook]
 * @return [hook]						[No return]
 */
function acs_tfs_init_translation() {
	load_plugin_textdomain( ACS_TFS_NAME , '', ACS_TFS_NAME . '/languages' );
}
add_action( 'plugins_loaded', 'acs_tfs_init_translation' );

/**
 * [acs_tfs_admin_css_js				BACK-OFFICE: Connect CSS styles and Java Script to Back-Office ]
 * @return [hook]                  		[No return]
 */
function acs_tfs_admin_css_js() {
	// CSS
	wp_register_style( 'acs_tfs_stylesheet_back', ACS_TFS_ASSETS_URL . 'css/acs-back-style.css' );
	wp_enqueue_style( 'acs_tfs_stylesheet_back' );

	// JS
	wp_register_script( 'acs_tfs_script_back', ACS_TFS_ASSETS_URL . 'js/acs-back-script.js', array( 'jquery' ) );
	wp_enqueue_script( 'acs_tfs_script_back' );
}
add_action( 'admin_enqueue_scripts', 'acs_tfs_admin_css_js' );

/**
 * [acs_tfs_front_css_js				FRONT-OFFICE: Connect Java Script to Front-Office ]
 * @return [hook]                  		[No return]
 */
function acs_tfs_front_css_js() {
	// CSS
	wp_register_style( 'acs_tfs_stylesheet_back', ACS_TFS_ASSETS_URL . 'css/acs-style.css' );
	wp_enqueue_style( 'acs_tfs_stylesheet_back' );

	// JS
	wp_register_script( 'acs_script', ACS_TFS_ASSETS_URL . 'js/acs-script.js', array( 'jquery' ) );
	wp_localize_script( 'acs_script', 'ajaxurl', admin_url('admin-ajax.p 16, 2017hp'));
	wp_enqueue_script( 'acs_script' );
}
add_action( 'wp_enqueue_scripts', 'acs_tfs_front_css_js' );

// OAuth
if (class_exists('TigrisSaleforceOAuth')) {
	global $acs_tfs_core_already_exists;
	$acs_tfs_core_already_exists = true;
}
else {
	require_once( plugin_dir_path( __FILE__ ) . 'class/TigrisSaleforceOAuth.php' );
}

$tigrisSaleforceOAuth = new TigrisSaleforceOAuth();

add_action( 'template_redirect','acs_tfs_redirect_to_listener');

function acs_tfs_redirect_to_listener() {
	global $options;
	...
	if (strpos($uri['path'], 'acs-tigris-for-salesforce/listener') === false ) {
		...
		if (function_exists('acs_tfs_save_listener')) {
...
/**
 * [acs_tfs_register_type_vacancy		CORE: Create Vacancy post type and taxonomy]
 * @return [hook]                  		[No return]
 */
function acs_tfs_register_type_vacancy() {
	global $options;
	...
	if ( isset( $options['category_off'] ) && $options['category_off'] ) {
		...
		$labels = array(
				'name'              => __( 'Vacancy Categories', ACS_TFS_NAME ),
				'singular_name'     => __( 'Vacancies Categories', ACS_TFS_NAME ),
				'search_items'      => __( 'Search Categories', ACS_TFS_NAME ),
				'all_items'         => __( 'All Vacancies Categories', ACS_TFS_NAME ),
				'parent_item'       => __( 'Parent Vacancies Categories', ACS_TFS_NAME ),
				'parent_item_colon' => __( 'Parent Vacancies Categories:', ACS_TFS_NAME ),
				'edit_item'         => __( 'Edit Category', ACS_TFS_NAME ),
				'update_item'       => __( 'Update Category', ACS_TFS_NAME ),
				'add_new_item'      => __( 'Add New Category', ACS_TFS_NAME ),
				'new_item_name'     => __( 'New Categories', ACS_TFS_NAME ),
				'menu_name'         => __( 'Vacancies Categories', ACS_TFS_NAME ),
				'popular_items'		=> __( 'Popular Vacancies', ACS_TFS_NAME ),
				'view_item'         => __( 'View Vacancy', ACS_TFS_NAME ),
				'view_items'		=> __( 'View Vacancy category', ACS_TFS_NAME )
			);
		$args	= array(
				'label'                 => __( 'Vacancies', ACS_TFS_NAME ),
				'labels'                => $labels,
				'description'           => __( 'Category for Tigris Vacancy', ACS_TFS_NAME ),
				'public'                => true,
		...
		$slug = 'vacatures/%tigrisvacancies%';
	}

	// Post type: vacatures
	$labels = array(
			'name'           	=> __( 'Vacancies', ACS_TFS_NAME ),
			'singular_name'  	=> __( 'Vacancy', ACS_TFS_NAME ),
			'menu_name'      	=> __( 'Tigris Vacancies', ACS_TFS_NAME ),
			'all_items'      	=> __( 'All Vacancies', ACS_TFS_NAME ),
			'add_new'        	=> _x( 'Add New', ACS_TFS_NAME ),
			'add_new_item'   	=> __( 'Add New Vacancy', ACS_TFS_NAME ),
			'edit'           	=> __( 'Edit', ACS_TFS_NAME ),
			'edit_item'      	=> __( 'Edit Vacancy', ACS_TFS_NAME ),
			'new_item'       	=> __( 'New Vacancy', ACS_TFS_NAME ),
			'view_item'       	=> __( 'View Vacancy', ACS_TFS_NAME ),
			'search_items'      => __( 'Search Vacancy', ACS_TFS_NAME ),
			'not_found'			=> __( 'Not found Vacancies', ACS_TFS_NAME ),
			'not_found_in_trash'=> __( 'Not found Vacancies in trash', ACS_TFS_NAME ),
			'parent_item_colon' => __( 'Parent Vacancy', ACS_TFS_NAME ),
			'parent_item_colon' => __( 'Parent Vacancy', ACS_TFS_NAME )
		);
	$args	= array(
			'label'               => __( 'Vacancy', ACS_TFS_NAME ),
			'labels'              => $labels,
			'description'         => '',
			'public'              => true,
...
add_action( 'init', 'acs_tfs_register_type_vacancy' );

/**
 * [acs_tfs_permalink_for_vacancy		CORE: Adding taxonomy to the CNC]
 * @param  [string] $permalink			[Current URL]
 * @param  [Object] $post     			[WpQuery]
 * @return [hook]                  		[No return]
 */
function acs_tfs_permalink_for_vacancy( $permalink, $post ) {
...
add_filter( 'post_type_link', 'acs_tfs_permalink_for_vacancy', 1, 2 );

/**
 * [acs_tfs_add_fields_for_vacancy		CORE: Adding services fields to Vacancy]
 * @return [hook]                  		[No return]
 */
function acs_tfs_add_fields_for_vacancy() {
	add_meta_box( 'vacancy_fields', __( 'Vacancy description', ACS_TFS_NAME ), 'acs_tfs_add_fields_box_for_vacancy', 'vacatures', 'normal', 'high'  );
}
add_action( 'add_meta_boxes', 'acs_tfs_add_fields_for_vacancy', 1 );

/**
 * [acs_tfs_add_fields_box_for_vacancy	BACK-OFFICE: Display fields box for vacancies]
 * @return [string]						[HTML code for services fields to Vacancy]
 */
function acs_tfs_add_fields_box_for_vacancy() {
	global $post;

	$fields = array(
		array(
			'name'	=> 'tigris__plaats__c',
			'label'	=> __( 'Location', ACS_TFS_NAME )
		),
		array(
			'name'	=> 'tigris__opleidingsniveau__c',
			'label'	=> __( 'Education level', ACS_TFS_NAME )
		),
		array(
			'name'	=> 'tigris__uren_per_week__c',
			'label'	=> __( 'Work week', ACS_TFS_NAME )
		),
		array(
			'name'	=> 'tigris__salaris_van__c',
			'label'	=> __( 'Salary level (min)', ACS_TFS_NAME )
		),
		array(
			'name'	=> 'tigris__salaris_tot__c',
			'label'	=> __( 'Salary level (max)', ACS_TFS_NAME )
		),
		array(
			'name'	=> 'tigris__functiegroep__c',
			'label'	=> __( 'Sector of economy', ACS_TFS_NAME )
		),
		array(
			'name'	=> 'tigris__soort_dienstverband__c',
			'label'	=> __( 'Type of employment', ACS_TFS_NAME )
		),
		array(
			'name'	=> 'tigris__branche__c',
			'label'	=> __( 'Branch', ACS_TFS_NAME )
		),
		array(
			'type'	=> 'checkbox',
			'name'	=> 'tigris__interne_vacature__c',
			'label'	=> __( 'Interior', ACS_TFS_NAME ),
...
/**
 * [acs_tfs_fields_updte_vacancy		CORE: Update data, while saving the post]
 * @param  [integer] $post_id			[ID current post]
 * @return [hook]                  		[No return]
 */
function acs_tfs_fields_updte_vacancy( $post_id ) {
	...
	// Correct
	if ( $old_url != $new_url ) {
		if (!class_exists('Plugin_Wp_Tigris_Rest'))
			require_once( plugin_dir_path( __FILE__ ) . 'class/class-plugin-wp-tigris-rest.php' );
		...
		$rest = new Plugin_Wp_Tigris_Rest();
...
add_action( 'save_post', 'acs_tfs_fields_updte_vacancy', 0 );
...
/**
 * [acs_tfs_register_type_branch 		BACK-OFFICE: Add custom Branch post type]
 * @return [hook]                   	[No return]
 */
function acs_tfs_register_type_branch() {
	// Post type: branch
	$labels = array(
			'name'           	=> __( 'Branches', ACS_TFS_NAME ),
			'singular_name'  	=> __( 'Branch', ACS_TFS_NAME ),
			'menu_name'      	=> __( 'Tigris Branches', ACS_TFS_NAME ),
			'all_items'      	=> __( 'All Branches', ACS_TFS_NAME ),
			'add_new'        	=> _x( 'Add New', ACS_TFS_NAME ),
			'add_new_item'   	=> __( 'Add New Branch', ACS_TFS_NAME ),
			'edit'           	=> __( 'Edit', ACS_TFS_NAME ),
			'edit_item'      	=> __( 'Edit Branch', ACS_TFS_NAME ),
			'new_item'       	=> __( 'New Branch', ACS_TFS_NAME ),
			'view_item'       	=> __( 'View Branch', ACS_TFS_NAME ),
			'search_items'      => __( 'Search Branch', ACS_TFS_NAME ),
			'not_found'			=> __( 'Not found Branches', ACS_TFS_NAME ),
			'not_found_in_trash'=> __( 'Not found Branches in trash', ACS_TFS_NAME ),
			'parent_item_colon' => __( 'Parent Branch', ACS_TFS_NAME ),
			'parent_item_colon' => __( 'Parent Branch', ACS_TFS_NAME )
		);
	$args	= array(
			'label'               => __( 'Branch', ACS_TFS_NAME ),
...
if ( ! isset( $options['branch_off'] ) ) {
	add_action( 'init', 'acs_tfs_register_type_branch' );
}
...
/**
 * [acs_tfs_add_fields_for_branch		CORE: Adding services fields to branches]
 * @return [hook]                  		[No return]
 */
function acs_tfs_add_fields_for_branch() {
	add_meta_box( 'branch_fields', __( 'Branch options', ACS_TFS_NAME ), 'acs_tfs_add_fields_box_for_branch', 'branch', 'normal', 'high'  );
}
if ( ! isset( $options['branch_off'] ) ) {
	add_action( 'add_meta_boxes', 'acs_tfs_add_fields_for_branch', 2 );
}

/**
 * [acs_tfs_add_fields_box_for_branch	BACK-OFFICE: Display fields box for Branch]
 * @return [string]						[HTML code for services fields to branches]
 */
function acs_tfs_add_fields_box_for_branch() {
	global $post;
	$options = get_option( str_replace( '-', '_', ACS_TFS_NAME ) );
	...
	$fields = array(
		...
		array(
			'name'		=> 'tigris__test_c',
			'label'		=> __( 'Street & house', ACS_TFS_NAME )
		),
		array(
			'name'		=> 'tigris__test_c2',
			'label'		=> __( 'ZIP & ​place', ACS_TFS_NAME )
		),
		array(
			'name'		=> 'tigris__test_d',
			'label'		=> __( 'E-mail', ACS_TFS_NAME )
		),
		array(
			'name'		=> 'tigris__test_e',
			'label'		=> __( 'Phone', ACS_TFS_NAME )
		),
		/*
		// Example:
		array(
			...
			'multiple'	=> TRUE,
			'name'		=> 'tigris__test_f',
			'label'		=> __( 'Employees', ACS_TFS_NAME ) . '<br><small>(' . __( 'multiselect', ACS_TFS_NAME ) . ')</small>',
		...
		<?php } elseif ( isset( $value['type'] ) && $value['type'] == 'select' ) { ?>
			...
			<p>
				...
				<select
					...
					<option><?php _e( 'Choose...', ACS_TFS_NAME ); ?></option>

					<?php
					if ( ! isset( $value['option'] ) || empty( $value['option'] ) ) {
						echo acs_tfs_generate_locations_option( get_post_meta( $post->ID, $value['name'], 1 ) );
					} else {
...
/**
 * [acs_tfs_fields_updte_branch			CORE: Update data, while saving the post]
 * @param  [integer] $post_id			[ID current post]
 * @return [hook]                  		[No return]
 */
function acs_tfs_fields_updte_branch( $post_id ) {
...
if ( ! isset( $options['branch_off'] ) ) {
	add_action( 'save_post', 'acs_tfs_fields_updte_branch', 1 );
}

/** Branch type: END */

/**
 * [acs_tfs_change_single_template		FRONT-OFFICE: Connect Plug-In page template]
 * @param  [string] $template			[Patch to WP-select template]
 * @return [string]          			[Patch to template]
 */
function acs_tfs_change_single_template( $template ) {
	...
	// Check plug-in pages
	if ( in_array( 'vacatures', $url_array ) ) {
		...
		if( taxonomy_exists( 'tigrisvacancies' ) ) {
			...
			if ( end( $url_array ) == 'vacatures' || ( isset( $vacancy_cat ) && in_array( end( $url_array ), $vacancy_cat ) ) ) {
				$category = 1;
			} else {
				$category = 0;
			}
		...
		if ( ( isset( $category ) && $category ) || ! is_single() ) {
			...
			if ( $new_template = locate_template( array( 'tigris/category-tigrisvacancies.php' ) ) ) {
				...
				$template = ACS_TFS_PLUGIN_DIR . '/templates/category-tigrisvacancies.php';
			}
		...
		// ... or single page
			...
			if ( $new_template = locate_template( array( 'tigris/single-tigrisvacancy.php' ) ) ) {
				...
				$template = ACS_TFS_PLUGIN_DIR . '/templates/single-tigrisvacancy.php';
			}
		}
	}
	...
	if ( in_array( 'branch', $url_array ) ) {
		...
		if ( $new_template = locate_template( array( 'tigris/single-tigrisbranch.php' ) ) ) {
			...
			$template = ACS_TFS_PLUGIN_DIR . '/templates/single-tigrisbranch.php';
		}
	}
	return $template;
}
add_filter( 'template_include', 'acs_tfs_change_single_template', 99 );

/**
 * [acs_tfs_load_vacancy				FRONT-OFFICE: AJAX loading vacations post]
 * @return [hook]                  		[HTML code next vacations posts]
 */
function acs_tfs_load_vacancy() {
	...
	if (  $_POST['action'] == 'loadvacancies' ) {
		...
		$acs_query = new WP_Query( $args );

		if( $acs_query->have_posts() ) {
			while( $acs_query->have_posts() ) {
				$acs_query->the_post();
				...
				if ( locate_template( array( 'tigris/ajax-tigrisvacancy.php' ) ) ) {
					...
					load_template( ACS_TFS_PLUGIN_DIR . '/templates/ajax-tigrisvacancy.php', false );
				}
			}
		}
...
add_action( 'wp_ajax_loadvacancies', 'acs_tfs_load_vacancy' );
add_action( 'wp_ajax_nopriv_loadvacancies', 'acs_tfs_load_vacancy' );

/**
 * [acs_tfs_add_form_shortcode			FRONT-OFFICE: Connection of feedback form]
 * @param  [array] $atts    			[Shortcode attributes]
 * 										vacancy=0 //Vacancy ID (0-without vacancy)
 */
function acs_tfs_add_form_shortcode( $atts ) {
	global $att;
	$att = $atts;
	...
	if ( locate_template( array( 'tigris/form-tigrisresume.php' ) ) ) {
		...
		load_template( ACS_TFS_PLUGIN_DIR . '/templates/form-tigrisresume.php', false );
	}
}
add_shortcode( 'tigris_form', 'acs_tfs_add_form_shortcode' );

function acs_tfs_add_single_form_shortcode( $atts ) {
	global $att;
	$att = $atts;
	...
	if ( locate_template( array( 'tigris/form-tigrisvacancy.php' ) ) ) {
		...
		load_template( ACS_TFS_PLUGIN_DIR . '/templates/form-tigrisvacancy.php', false );
	}
}
add_shortcode( 'tigris_single_form', 'acs_tfs_add_single_form_shortcode' );

/**
 * [acs_tfs_load_form					FRONT-OFFICE: AJAX sending a job application]
 * @return [hook]                  		[Result of processing the form]
 */
	function acs_tfs_load_form() {
	$reply = 0;
	if ( isset( $_POST['action'] ) && $_POST['action'] == 'formvacancies' && wp_verify_nonce( $_POST['_wpnonce'] ) ) {
		...
		if ( !empty($responce) ) {
			...
			$reply = __( 'The application for the job was sent successfully.', ACS_TFS_NAME );
		...
		if ( ! $responce || ( isset( $options['email_on'] ) && $options['email_on'] ) ) {
			...
			if ( !empty($responce) ) {
				$status = __( 'The vacancies were successfully sent to Tigris.', ACS_TFS_NAME );
			} else {
				$status = __( 'There was a failure to send a job application for the Salesforce service.', ACS_TFS_NAME );
			}
			...
			if ( $options['email'] ) {
				...
				$subject = $status;
				$message = '<br>'
						 . '<p>' . __( 'Job vacancy', ACS_TFS_NAME ) . ': <a href="' .esc_url($post['url']) . '">' . esc_html($post['title']) . '</a></p>'
						 . '<p>' . __( 'Applicant`s firstname', ACS_TFS_NAME ) . ': ' . esc_html( $_POST['firstname'] ) . '</p>'
						 . '<p>' . __( 'Applicant`s lastname', ACS_TFS_NAME ) . ': ' . esc_html( $_POST['lastname'] ) . '</p>'
						 . '<p>' . __( 'Applicant`s e-mail', ACS_TFS_NAME ) . ': ' . esc_html( $_POST['email'] ) . '</p><br>'
						 . '<p>' . __( 'Applicant`s message', ACS_TFS_NAME ) . ': ' . esc_html( $_POST['your-message'] ) . '</p>'
						 . '<br><hr><br>'
						 . '<p>' . __( 'Referer URL', ACS_TFS_NAME ) . ': ' . get_home_url() . esc_url( $_POST['_wp_http_referer'] ) . '</p><br>';
				...
				if ( wp_mail( $to, $subject, $message, $headers, $attachments ) || !empty($responce) ) {
					...
					$reply = __( 'Send failed. Please try again later.', ACS_TFS_NAME );
				}
			}
		}
	}
...
add_action( 'wp_ajax_formvacancies', 'acs_tfs_load_form' );
add_action( 'wp_ajax_nopriv_formvacancies', 'acs_tfs_load_form' );

/** Tigris-author role: BEGIN */

/**
 * [acs_tfs_add_tigris_role				CORE: Add Tigris-author role]
 */
function acs_tfs_add_tigris_role() {
	$result = add_role( 'tigris_author', __( 'Tigris author', ACS_TFS_NAME ),
		array(
...
/**
 * [acs_tfs_remove_tigris_role			CORE: Remove Tigris-author role]
 */
function acs_tfs_remove_tigris_role() {
	remove_role( 'tigris_author' );
}

/**
 * [acs_tfs_profile_fields 				CORE: Add new field for Tigris-author profile]
 * @param  [object] $user 				[User object]
 */
function acs_tfs_profile_fields( $user ) {
	...
	$button_text     = $upload_url ? __( 'Change Current Image', ACS_TFS_NAME ) :  __( 'Upload New Image', ACS_TFS_NAME );

	if ( $upload_url ) {
		$upload_edit_url = get_site_url() . $upload_edit_url;
	} ?>

	<h3><?php _e( 'Employee Information', ACS_TFS_NAME ); ?></h3>
	<table class="form-table">
		<tr>
			<th>
				<label for="employee_linkedin">
					<?php _e( 'LinkedIn', ACS_TFS_NAME ); ?>
				</label>
			</th>
			<td>
				<input
		...
		<tr>
			<th>
				<label for="employee_phone_office">
					<?php _e( '​Phone number (office)', ACS_TFS_NAME ); ?>
				</label>
			</th>
			<td>
				<input
		...
		<tr>
			<th>
				<label for="employee_phone_mobile">
					<?php _e( '​Phone number (mobile)', ACS_TFS_NAME ); ?>
				</label>
			</th>
			<td>
				<input
		...
		<tr>
			<th>
				<label for="employee_description">
					<?php _e( 'Description', ACS_TFS_NAME ); ?>
				</label>
			</th>
			<td>
...
if ( ! isset( $options['roles_off'] ) ) {
	add_action( 'show_user_profile', 'acs_tfs_profile_fields' );
	add_action( 'edit_user_profile', 'acs_tfs_profile_fields' );
}

/**
 * [acs_tfs_save_profile_fields 		CORE: Save update value]
 * @param  [integer] $user_id 			[User ID]
 */
function acs_tfs_save_profile_fields( $user_id ) {
...
if ( ! isset( $options['roles_off'] ) ) {
	add_action( 'personal_options_update', 'acs_tfs_save_profile_fields' );
	add_action( 'edit_user_profile_update', 'acs_tfs_save_profile_fields' );
}

/**
 * [acs_tfs_checked_tigris_author		CORE: Checking and creating the author Tigris]
 * @param  [string] $username			[Author name: field 'tigris__owner_name__c']
 * @return [integer]          			[Author ID]
 */
function acs_tfs_checked_tigris_author( $username ) {
	...
	if ( ! $user_id = username_exists( $user_str ) ) {
		...
		$userdata = array(
			...
			'description'     		=> __( 'The user is created automatically.', ACS_TFS_NAME ),
			'rich_editing'    		=> 'false',
...
/**
 * [acs_tfs_crate_vacancies_category 	CORE: Checking and creating vacancies category]
 ...
 */
function acs_tfs_crate_vacancies_category( $category_name ) {
	global $options;
	...
	if ( isset( $options['category_off'] ) && $options['category_off'] ) {
		...
		if ( empty( $categories ) ) {
			...
			$data = wp_insert_term(
				...
				$tax,
				array(
					'description' => __( 'The category is created automatically.', ACS_TFS_NAME ),
					'slug'        => $slug
				)
			);
			if( ! is_wp_error($data) )
...
/**
 * [acs_tfs_added_tigristag 			CORE: Added tags to vacancy]
 ...
 */
function acs_tfs_add_tigristag( $postID, $tags = array() ) {
	if ( empty( $tags ) ) {
		return false;
	}
...
/**
 * [acs_tfs_update_tigristag 			CORE: Update vacancies tags]
 * @param  [integer] $postID 			[Vacancy ID]
 * @param  [array] $tags 				[Tags for vacancy: fields 'tigris__soort_vacature__c', 'tigris__interne_vacature__c'  and 'tigris__plaats__c']
 * @return [boolean]          			[Operation status]
 */
function acs_tfs_update_tigristag( $postID, $tags = array() ) {
...
/**
 * [acs_tfs_generate_locations_option 	FRONT-OFFICE: Generate options for selecting a location]
 * @param  [string] $selected 			[Selected option]
 * @return [mixed]          			[HTML code or false - when locations does not exist.]
 */
function acs_tfs_generate_locations_option( $selected = false ) {
	global $options;
 ...
 * 										page=1 //Page Number
 */
function acs_tfs_add_vacancy_list_shortcode( $atts ) {
	ob_start();
	global $att;
	$att = $atts;
	...
	while ( $vacancies->have_posts() ) {
		...
		if ( locate_template( array( 'tigris/vacancy_list.php' ) ) ) {
			...
			load_template( ACS_TFS_PLUGIN_DIR . '/templates/vacancy_list.php', false );
		}
	}
	if (!isset($att['is_ajax'])) {
		$att['action'] = 'tigris_vacancy_list';
		echo '</div>';
		if ($vacancies->post_count >= $att['count']) {?>
		<div class="tigris-more-vacancies">
			<a href="#" class="js-tigris-more-vacancies" data-page="1"><?php echo __('More Vacancies', ACS_TFS_NAME)?></a>
		</div>
		              ...
		              $.ajax({
		                ...
		                success:function(data){
		                  	if( data) {
			                	...
			                	if (data.count < <?php echo $att['count']?>) {
			                		that.parent().remove();
			              		}
			              	}
		                }
	...
	<?php
		...
		$vacancyList['count'] = $vacancies->post_count;
		$vacancyList = json_encode($vacancyList);
	}
	
    return $vacancyList;
}
add_shortcode( 'tigris_vacancy_list', 'acs_tfs_add_vacancy_list_shortcode' );

add_action('wp_ajax_tigris_vacancy_list', 'ajax_tigris_vacancy_list');
add_action('wp_ajax_nopriv_tigris_vacancy_list', 'ajax_tigris_vacancy_list');

function ajax_tigris_vacancy_list() {
	$shortCodeAttr = '';
 ...
 */
function acs_tfs_add_searchform_shortcode($atts) {
	global $att;
	global $options;
	$att = $atts;
	...
	$html_code = '<form id="tigris-searchform" role="search" method="get" class="tigris-search-form" action="'.home_url( '/' ).'">
		...
		<label class="tigris-search-form-label-find-job">
			<input
				...
				placeholder="'.esc_attr_x('Find a job', ACS_TFS_NAME).' …"
		...
		<input type="text" class="js-search-form-item tigris-search-form-location" name="tigris_location" placeholder="'.__('Region, city or zip code', ACS_TFS_NAME).'" value="' . $get_val['tigris_location'] . '">';
	if (isset($options['google_api_key']) && !empty($options['google_api_key']) && isset($att['distances']) && !empty($att['distances'])) {
		$html_code .= '<select name="distance" class="js-search-form-item tigris-search-form-distances">';
		$html_code .= '<option value="0">' . __('Distance', ACS_TFS_NAME) . '</option>';
		...
		foreach ($distances as $distance) {
			...
			$html_code .= '<option value="' . $distance . '"' . $selected . '>' . $distance . ' '. __('km', ACS_TFS_NAME) . '</option>';
		}
		$html_code .= '</select>';
	}
	$html_code .= '</div>';
	...
	$html_code .= sprintf(__( 'All %s vacancies', ACS_TFS_NAME ), '<strong>' . $total . '</strong>');
	$html_code .= '</button></div></form>';
	return $html_code;
}

add_shortcode( 'tigris_searchform', 'acs_tfs_add_searchform_shortcode' );
 ...
 * 									distances=15,20,25 //distance
 */
function acs_tfs_add_filter_shortcode($atts) {
	global $wpdb;
	global $att;
	global $options;
	$att = $atts;
	...
	$html_code = '<form id="tigris-filter-form" role="search" method="get" class="tigris-filter-form" action="' . home_url( '/' ) . '">
					...
					<ul class="tigris-filter-first">
						<li class="transition">
							...
							<span>' . __('Function', ACS_TFS_NAME) . '</span>
							<ul class="tigris-filter-second">
								<li>
									<input type="text" class="js-filter-form-item" name="s" placeholder="' . __('Find a job', ACS_TFS_NAME) . ' …" value="' . $get_val['s'] . '">
								</li>
							</ul>
						</li>
						<li class="transition">
							<i class="transition"></i>
							<span>' . __('Place', ACS_TFS_NAME) . '</span>
							<ul class="tigris-filter-second">
								<li>
									<input type="text" class="js-filter-form-item" name="tigris_location" placeholder="' . __('Region, city or zip code', ACS_TFS_NAME) . '" value="' . $get_val['tigris_location'] . '">
								</li>
							</ul>
						</li>';
						if (isset($options['google_api_key']) && !empty($options['google_api_key']) && isset($att['distances']) && !empty($att['distances'])) {
						...
						<li class="transition">
							<i class="transition"></i>
							<span>' . __('Education level', ACS_TFS_NAME) . '</span>
							<ul class="tigris-filter-second">
								<li' . ( ! empty( $get_val['education'] ) ? ' class="selected"' : '' ) . '>
									<select name="education" class="tigris-filter-education js-filter-form-item">';
										if ( ! empty( $tigris_array['tigris__opleidingsniveau__c'] ) ) {
											$html_code .= '<option value="0">' . __('Choose...', ACS_TFS_NAME) . '</option>';
		...
		$html_code .= '
						<li class="transition">
							<i class="transition"></i>
							<span>' . __( 'Work week', ACS_TFS_NAME) . '</span>
							<ul class="tigris-filter-second">
		...
		$html_code .= '
						<li class="transition">
							<i class="transition"></i>
							<span>' . __('Branch', ACS_TFS_NAME) . '</span>
							<ul class="tigris-filter-second">
					...
					<button type="submit" id="filtersubmit" class="tigris-filter-submit button-red transition js-tigris-filter">' . __('Search', ACS_TFS_NAME) . '</button>
					<button type="reset" class="tigris-filter-clear button-white transition js-tigris-reset">' . __('Clear filters', ACS_TFS_NAME) . '</button>
				</form>';
	return $html_code;
}


add_shortcode( 'tigris_filter', 'acs_tfs_add_filter_shortcode' );
/**
 * [tigris_vacansies_search 				Filter requests for search result]
 * @param  [object] $query 					[WP_Query object]
 * @return [hook]                   		[No return]
 */
function acs_tfs_vacansies_search( $query ) {
	...
	if ( $query->is_search && isset( $_GET['post__in'] ) && $_GET['post__in'] ) {
		...
		if ( $query->is_search && ( isset( $_GET['distance'] ) && ! empty( $_GET['distance'] ) && $_GET['distance'] ) ) {
			...
			if($location === false) {
			$center = acs_tfs_get_latlng(false);
			} else {
				$center = acs_tfs_get_latlng($location);
			}
	        $post__in = [];
	        if( $center !== false){
	            if(isset($args['meta_query'][0]['value']))
	                unset($args['meta_query'][0]['value']);
	            $query_vacatures = new WP_Query( ['post_type' => 'vacatures', 'posts_per_page' => -1] );
		        while ( $query_vacatures->have_posts() ) {
			        $query_vacatures->the_post();
			        $latlng = acs_tfs_get_or_update_latlng(get_the_ID());
			        if($latlng === false) continue;

			        $d = acs_tfs_get_distance($latlng, $center) / 1000;
...
add_action( 'pre_get_posts', 'acs_tfs_vacansies_search' );

/**
 * @param $address | String or false
 * @return mixed | Array or false
 */
function acs_tfs_get_latlng($address){
	global $options;
	if ($address) {
 ...
 * @return array
 */
function acs_tfs_get_or_update_latlng($post_id){
	...
	if($geo_lat == 0 || $geo_lng == 0){
		...
		if(count($address) == 0)
			return false;
		$latlng = acs_tfs_get_latlng($address[0]);
		if($latlng !== false) {
 ...
 * @return int
 */
function acs_tfs_get_distance($point1, $point2){
...
/**
 * [acs_tfs_search_correctcount 			FRONT-OFFICE: Vacancy location for search]
 * @return [string] 						[HTML code]
 */
function acs_tfs_search_correctcount() {
...
add_action( 'wp_ajax_correctcount', 'acs_tfs_search_correctcount' );
add_action( 'wp_ajax_nopriv_correctcount', 'acs_tfs_search_correctcount' );

/**
 * [acs_tfs_search_autocomplete 			FRONT-OFFICE: Autocomlete for search]
 * @return [string] 						[HTML code]
 */
function acs_tfs_search_autocomplete() {
...
add_action( 'wp_ajax_autocomlete', 'acs_tfs_search_autocomplete' );
add_action( 'wp_ajax_nopriv_autocomlete', 'acs_tfs_search_autocomplete' );