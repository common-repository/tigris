 ...
 * @since 1.0
 */
class TigrisSaleforceOAuth {

	public function __construct() {
		add_action( 'login_form', array( &$this, 'acs_tfs_salesforce_login_form' ) );
		add_action( 'template_redirect', array( &$this, 'acs_tfs_template_redirect' ) );
		add_action( 'wp_logout', array( &$this, 'cs_tfs_tigris_logout' ), 1, 2 );
	}
	 ...
	 * @return void
	 */
	public function acs_tfs_salesforce_login_form() {
		if ( isset( $_GET['tigris-oauth-error'] ) ) {
			$description = !empty($_GET['description']) ? sanitize_text_field($_GET['description']) : __('Error connecting to Saleforce.', ACS_TFS_NAME);
			...
			echo '<div style="padding:10px;background-color:#FFDFDD;border:1px solid #ced9ea;border-radius:3px;-webkit-border-radius:3px;-moz-border-radius:3px;"><p style="line-height:1.6em;"><strong>Error!</strong> ' . sprintf(__( 'User %s  is not authorised to login.', ACS_TFS_NAME ), $username) .' </p></div><br>';
		}
		echo ' <style type="text/css">
			...
			#loginform .sale-force-button a {
				...
				background-image: url("'. ACS_TFS_ASSETS_URL . 'img/plugin-icon.png");
	...
	public function acs_tfs_template_redirect() {
			...
			if ($code) {
					...
					if ( isset( $oauth_response['access_token'] ) ) {
						...
						if (is_user_logged_in()) {
							...
							wp_redirect('/wp-admin/admin.php?page=acs-tigris-for-salesforce');
						} else {
							$user = get_user_by('login', $oauth_username);
							
							if (! isset( $user->ID ) ) {
								$new_user_id = $this->acs_tfs_try_create_user($oauth_username);
	...
	protected function  acs_tfs_try_create_user($username) {
	 ...
	 * @return void
	 */
	public function cs_tfs_tigris_logout() {
	...
	public static function acs_tfs_salesforce_get_token() {
		$optionName = str_replace( '-', '_', ACS_TFS_NAME );
	...
	public static function validateToken($oauth_response) {
		...
		if( ! is_wp_error($idtoken_validation_result) && $idtoken_validation_result['response']['code'] == 200) {
		...
		return array($oauth_username, $idtoken_response['urls']['rest']);
	}
}
?>