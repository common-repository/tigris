	...
	public function __construct() {
		...
		if (!isset( $_COOKIE['tigris_oauth_access_token'] ) || !$_COOKIE['tigris_oauth_access_token'] || $expired) {
			if (!class_exists('TigrisSaleforceOAuth'))
				require_once( plugin_dir_path( __FILE__ ) . 'TigrisSaleforceOAuth.php' );
			if (isset($options['refresh_token']) && !empty($options['refresh_token'])) {
				list($this->_token, $this->_restUrl) = TigrisSaleforceOAuth::acs_tfs_salesforce_get_token();
			}
		}
	}
