...
<!-- Default template from Plug-In category-tigrisvacancy.php: BEGIN -->
	<section class="vacancies-page__block">
		...
		<div class="vacancies-page__title">
			<h1 class="h1">
				<?php echo wp_sprintf( __( 'Search our %s vacancies', ACS_TFS_NAME ), wp_count_posts( 'vacatures' )->publish ); ?>
			</h1>
		</div>
			...
			<div class="vacancies-page__vacancies-block">
				...
				if (  $vacancies->max_num_pages > 1 ) { ?>
					<div id="vacancy-load">
						<span data-load="<?php _e( 'Loading...', ACS_TFS_NAME ); ?>">
							<?php _e( 'Show more', ACS_TFS_NAME ); ?>
						</span>
					</div>
					<script>
